...
class Seq:
    ...
    def subset_cram_to_fastq(self, cram_file=['HPAP001.cram'], fasta_file='Homo_sapiens_assembly38.fasta', bed_file='keep_chroms.bed', out_file='subset_cram_to_fastq.sh', n_thresholds=2):
        '''
    ...
    def run_hlahd(self, fq1_file=['HPAP001_subset.R1.fastq.gz'], freq_data='freq_data', dict_file='dictionary', gene_split_file='HLA_gene.split.txt', out_file='run_hlahd.sh', out_dir='HLA-HD', n_thresholds=2, min_read_length=100, trim_rate=0.95):
    ...
    def run_xhla(self, fq1_file=['HPAP001_subset.R1.fastq.gz'], sif_file='xhla.sif', fasta_file='Homo_sapiens_assembly38.fasta', out_file='run_xhla.sh', n_thresholds=2, out_dir='xHLA'):
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
